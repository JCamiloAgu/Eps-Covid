...
namespace CitasEps.Views.Quote {
	public partial class CreateDeleteQuote : Form {
		...
		public CreateDeleteQuote(Func<bool, object> OnCloseForm, string officialId, string quoteId, DateTime quoteDateTime) {
			InitializeComponent();
			lblName.Hide();
			lblNameUser.Text = "";
		...
		private Models.Quote BuildQuote(int quoteId = 0) {
			...
			if (newAffiliate != null) {
				Models.User user = usersController.Get("id_affiliates", newAffiliate.GetAttribute("id").ToString());
